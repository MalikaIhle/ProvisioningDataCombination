...
\textcolor{blue}{likely not, but do one model per each category of age would remove this correlation - see below}  
...
\textcolor{blue}{repeatability of BirdID, SocialPartnerID, PairID: female more repeatable over partners than the pair alternation is repeatable ??  variance/regularity in interfeed interval affects the ability for the partner to adjust}
...
## \textcolor{blue}{model Age6} with DVDInfoChickNb with both PairBroodNb and ParentAge

```{r, echo=FALSE}
summary(modA_Age6)
```

PairID has the explain the most variance, second most is MumID

## \textcolor{blue}{model Age10} with DVDInfoChickNb with both PairBroodNb and ParentAge

```{r, echo=FALSE}
summary(modA_Age10)
```

PairID explain absolutely no variance, the most variance is explained by MumID

## Model assumption checking for \textcolor{green}{full model with DVDInfoChickNb (both ages)}
...
_Nakagawa et al 2007_

provisioning data were not always
normally distributed. Therefore, prior to statistical tests,
appropriate transformations were identified and applied
using the Box–Cox transformation to achieve normality.  

* how ?? boxcox() only work for lm not lmer !

The within-year and between-year repeatabilities
(2004 and 2005) for feeding visits (visits/chick h) were calculated using a
procedure using within- and between-variance components
in a linear mixed effects model (LMM) with the
restricted maximum-likelihood method (REML, nlme
package in R; Pinheiro & Bates, 2000) with bird identity
as a grouping random factor in the model.
both male and female feeding visits per
chick also tended to decline with increasing brood size
(Nakagawa et al., 2007a), so these effects were controlled
for.  
Statistical significance
of repeatability was estimated from a likelihood
ratio test comparing deviances between an LMM without
a grouping random factor (i.e. individual identity) and
an LMM with this factor. The use of the parametric bootstrap
for likelihood ratios, rather than the chi-squared
distribution, is recommended because likelihood ratios
obtained in LMM often do not follow chi-squared
distributions and the bootstrap approach provides more
accurate approximation of P values (Faraway, 2006).
Therefore, we used parametric bootstraps to create the
distributions of likelihood ratios (5000 times) and the
observed likelihood ratios are compared against these
distributions to obtain P values  

* is this what the rptR package is supposed to be doign autocatically ?
