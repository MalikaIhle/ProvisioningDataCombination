...
library(here)
source(paste(here(),'Coordination_DataAnalyses.R', sep='/'))
```
...
drop1(modChickSurvival, test="Chisq") # Likelihood ratio test
dispersion_glmer(modChickSurvival) # from package blmeco
```


## Divorce correlates

They are 37 polygynous males, for which a total of 67 brood the male divorce (breed with another female), while the female does not (her next brood will be with the same male again).
In the first model, the dependent variable 'PairDivorce' is NA (those broods are excluded), in the second, PairDivorce is set to FALSE (female perspective), and in the third model, PairDivorce is set to TRUE (male perspective).
...
dispersion_glmer(mod_Divorce_PolygynousDontDivorce)

summary(mod_Divorce_PolygynousDivorce) 
drop1(mod_Divorce_PolygynousDivorce, test = "Chisq")
dispersion_glmer(mod_Divorce_PolygynousDivorce)
